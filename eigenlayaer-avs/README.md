...
2) RoboticAgent contract receives the request and emits a NewTaskCreated event for the request.
...
4) Each Operator submits their signed hash back to the RoboticAgent AVS contract.
...
- Users who wish to build an AVS for Production purposes will want to migrate from the `ECDSAServiceManagerBase` implementation in `RoboticAgentServiceManager.sol` to a BLS style architecture using [RegistryCoordinator](https://github.com/Layr-Labs/eigenlayer-middleware/blob/dev/docs/RegistryCoordinator.md).
