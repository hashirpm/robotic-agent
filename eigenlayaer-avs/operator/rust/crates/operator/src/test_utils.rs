...
use hello_world_utils::{
    ...
    helloworldservicemanager::{RoboticAgentServiceManager, IRoboticAgentServiceManager::Task},
...
async fn monitor_new_tasks() -> Result<()> {
    ...
    loop {
        ...
        for log in logs {
            match log.topic0() {
                Some(&RoboticAgentServiceManager::NewTaskCreated::SIGNATURE_HASH) => {
                    let RoboticAgentServiceManager::NewTaskCreated { taskIndex, task } = log
                        .log_decode()
...
mod tests {
    ...
    use std::path::Path;
    use RoboticAgentServiceManager::latestTaskNumReturn;
    #[tokio::test]
    #[serial]
    ...
    async fn test_spam_tasks() {
        ...
        let parsed: RoboticAgentData = serde_json::from_str(&data).unwrap();
        ...
        let hello_world_contract =
            RoboticAgentServiceManager::new(hello_world_contract_address, provider);
