...
async function loadDeployments(): Promise<Record<string, any>> {
  ...
  const [coreDeployment, roboticAgentDeployment] = await Promise.all([
    ...
    loadJsonFile(roboticAgentFilePath)
  ]);

  if (!coreDeployment || !roboticAgentDeployment) {
  ...
  return {
    core: coreDeployment,
    roboticAgent: roboticAgentDeployment
  };
}
...
describe('Operator Functionality', () => {
  ...
  let roboticAgentServiceManager: ethers.Contract;
  ...
  beforeAll(async () => {
    ...
    const roboticAgentServiceManagerABI = await loadJsonFile(path.join(__dirname, '..', 'abis', 'RoboticAgentServiceManager.json'));
    ...
    roboticAgentServiceManager = new ethers.Contract(deployment.roboticAgent.addresses.roboticAgentServiceManager, roboticAgentServiceManagerABI, signer);
    ecdsaRegistryContract = new ethers.Contract(deployment.roboticAgent.addresses.stakeRegistry, ecdsaRegistryABI, signer);
  ...
  it('should register operator to AVS', async () => {
    ...
    const operatorDigestHash = await avsDirectory.calculateOperatorAVSRegistrationDigestHash(
      signer.address,
      await roboticAgentServiceManager.getAddress(),
      salt,
      expiry
    );
