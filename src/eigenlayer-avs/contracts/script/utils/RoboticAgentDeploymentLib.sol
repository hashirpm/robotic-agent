...
library RoboticAgentDeploymentLib {
    using stdJson for *;
    using Strings for *;
    ...
    struct DeploymentData {
        address roboticAgentServiceManager;
    ...
    function deployContracts(
        ...
        result.roboticAgentServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        // Deploy the implementation contracts, using the proxy contracts as inputs
        address stakeRegistryImpl =
            address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address roboticAgentServiceManagerImpl = address(
            new RoboticAgentServiceManager(
                core.avsDirectory, result.stakeRegistry, core.rewardsCoordinator, core.delegationManager
            )
        );
        // Upgrade contracts
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (result.roboticAgentServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);
        upgradeCall = abi.encodeCall(RoboticAgentServiceManager.initialize, (owner, rewardsInitiator));
        UpgradeableProxyLib.upgradeAndCall(result.roboticAgentServiceManager, roboticAgentServiceManagerImpl, upgradeCall);
    ...
    function readDeploymentJson(
        ...
        require(vm.exists(fileName), "RoboticAgentDeployment: Deployment file does not exist");

        string memory json = vm.readFile(fileName);

        DeploymentData memory data;
        /// TODO: 2 Step for reading deployment json.  Read to the core and the AVS data
        data.roboticAgentServiceManager = json.readAddress(".addresses.roboticAgentServiceManager");
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.roboticAgentServiceManager));
    ...
    function readDeploymentConfigValues(
        ...
        require(vm.exists(pathToFile), "RoboticAgentDeployment: Deployment Config file does not exist");
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            '","roboticAgentServiceManager":"',
            data.roboticAgentServiceManager.toHexString(),
            '","roboticAgentServiceManagerImpl":"',
            data.roboticAgentServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
