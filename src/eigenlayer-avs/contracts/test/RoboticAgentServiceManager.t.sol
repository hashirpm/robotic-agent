...
import {RoboticAgentServiceManager} from "../src/RoboticAgentServiceManager.sol";
import {MockAVSDeployer} from "@eigenlayer-middleware/test/utils/MockAVSDeployer.sol";
import {ECDSAStakeRegistry} from "@eigenlayer-middleware/src/unaudited/ECDSAStakeRegistry.sol";
import {Vm} from "forge-std/Vm.sol";
import {console2} from "forge-std/Test.sol";
import {RoboticAgentDeploymentLib} from "../script/utils/RoboticAgentDeploymentLib.sol";
...
import {IRoboticAgentServiceManager} from "../src/IRoboticAgentServiceManager.sol";
import {ECDSAUpgradeable} from
    "@openzeppelin-upgrades/contracts/utils/cryptography/ECDSAUpgradeable.sol";

contract RoboticAgentTaskManagerSetup is Test {
    ...
    RoboticAgentDeploymentLib.DeploymentData internal roboticAgentDeployment;
    ...
    function setUp() public virtual {
        ...
        roboticAgentDeployment =
            RoboticAgentDeploymentLib.deployContracts(proxyAdmin, coreDeployment, quorum, owner.key.addr, owner.key.addr);
        labelContracts(coreDeployment, roboticAgentDeployment);
    }
    ...
    function labelContracts(
        ...
        RoboticAgentDeploymentLib.DeploymentData memory roboticAgentDeployment
    ) internal {
        ...
        vm.label(roboticAgentDeployment.roboticAgentServiceManager, "RoboticAgentServiceManager");
        vm.label(roboticAgentDeployment.stakeRegistry, "StakeRegistry");
    }
    ...
    function registerOperatorToAVS(
        ...
        ECDSAStakeRegistry stakeRegistry = ECDSAStakeRegistry(roboticAgentDeployment.stakeRegistry);
        ...
        bytes32 operatorRegistrationDigestHash = avsDirectory
            ...
            operator.key.addr, address(roboticAgentDeployment.roboticAgentServiceManager), salt, expiry
        );
    ...
    function createTask(TrafficGenerator memory generator, string memory taskName) internal {
        IRoboticAgentServiceManager roboticAgentServiceManager =
            IRoboticAgentServiceManager(roboticAgentDeployment.roboticAgentServiceManager);

        vm.prank(generator.key.addr);
        roboticAgentServiceManager.createNewTask(taskName);
    }

    function respondToTask(
        Operator memory operator,
        IRoboticAgentServiceManager.Task memory task,
        uint32 score,
        uint32 referenceTaskIndex
    ) internal {
        // Create the message hash
        bytes32 messageHash = keccak256(abi.encodePacked(score));
        ...
        IRoboticAgentServiceManager(roboticAgentDeployment.roboticAgentServiceManager).respondToTask(
            task, score, referenceTaskIndex, signedTask
        );
    }
}

contract RoboticAgentServiceManagerInitialization is RoboticAgentTaskManagerSetup {
    function testInitialization() public view {
        ...
        assertTrue(roboticAgentDeployment.stakeRegistry != address(0), "StakeRegistry not deployed");
        assertTrue(
            roboticAgentDeployment.roboticAgentServiceManager != address(0),
            "RoboticAgentServiceManager not deployed"
        );
...
contract RegisterOperator is RoboticAgentTaskManagerSetup {
    ...
    IRoboticAgentServiceManager internal sm;
    ...
    function setUp() public virtual override {
        ...
        sm = IRoboticAgentServiceManager(roboticAgentDeployment.roboticAgentServiceManager);
        stakeRegistry = ECDSAStakeRegistry(roboticAgentDeployment.stakeRegistry);
...
contract CreateTask is RoboticAgentTaskManagerSetup {
    IRoboticAgentServiceManager internal sm;
    ...
    function testCreateTask() public {
        ...
        vm.prank(generator.key.addr);
        IRoboticAgentServiceManager.Task memory newTask = sm.createNewTask(taskName);

        require(sha256(abi.encodePacked(newTask.username)) == sha256(abi.encodePacked(taskName)), "Task name not set correctly");
        require(newTask.taskCreatedBlock == uint32(block.number), "Task created block not set correctly");
    }
}

contract RespondToTask is RoboticAgentTaskManagerSetup {
    ...
    function testRespondToTask(uint32 score) public {
        ...
        sm.respondToTask(newTask, score, taskIndex, signedTask);
    }
}